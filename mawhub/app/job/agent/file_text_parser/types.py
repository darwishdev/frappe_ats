...
class ValidationRequestModel(BaseModel):
    full_text: str = Field(
        description="Full original document text."
    )

    file_path: Optional[str] = Field(
        default=None,
        description="Original file path if available."
    )

    file_hash: Optional[str] = Field(
        default=None,
        description="Original file hash if available."
    )

    metadata: dict = Field(
        description="Extracted metadata as key-value pairs."
    )
    sections: List[ParsedSectionModel] = Field(
        description="Extracted structured sections from parser."
    )
class ValidationResponseModel(BaseModel):
    status: Literal["accepted", "corrected"] = Field(
        ...,
        description="accepted if extraction is fully correct, corrected if modifications were required."
    )

    data: Optional[List[ParsedSectionModel]] = Field(
        default=None,
        description="Final validated structured document."
    )
    metadata: Optional[List[DocumentMetaFieldModel]] = Field(
        default=None,
        description="Correctd metadata"
    )
